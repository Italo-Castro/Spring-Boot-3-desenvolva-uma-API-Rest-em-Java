...
public class MedicoController {
    ...
    @Autowired // indica que o spring deve intanciar o repostory .. injeção dependencia
    private MedicoRepository repository;

    @PostMapping
    @Transactional
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastroMedico dados, UriComponentsBuilder uri) {
        var medico = new Medico(dados);
        repository.save(medico);
        var url = uri.path("/medicos/${id}").buildAndExpand(medico.getId()).toUri();
